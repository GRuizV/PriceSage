...
def run(config_path=None, only=None, store=True) -> list[PriceObservation]:
...
def main() -> None:
    ...
    parser.add_argument("--no-store", action="store_true", help="collect but don't write to data/raw")
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s: %(message)s")
    observations = run(config_path=args.config, only=args.vendor, store=not args.no_store)
